...
export const ProjectCard = ({ project }: { project: Project }) => {
  return (
    <motion.div layoutId={`project-${project.title}`} className='flex justify-between items-center gap-2 cursor-pointer group/project'>
      <div className='flex items-center gap-1 w-[80%]'>
        <motion.p layoutId={project.title} className='font-bold text-nowrap'>{project.title}</motion.p>
      ...
      <div className='p-3 w-full flex flex-col justify-between flex-1'>
        <div>
          <motion.h1
            ...
            className='font-sans tracking-tight font-semibold text-xl'
          >
            {project.title}
          </motion.h1>
          <motion.div
